...
function parsetest(x, wb, full, ext) {
    ...
    describe(`${x + ext} should have all bits`, function () {
        ...
        if (!fs.existsSync(sname)) {
            sname = `${dir}2011/${x.substr(x.lastIndexOf('/') + 1)}.sheetnames`
        }
        if (!fs.existsSync(sname)) {
            sname = `${dir}2013/${x.substr(x.lastIndexOf('/') + 1)}.sheetnames`
        }
...
describe.skip('should parse test files', function () {
    files.forEach(function (x) {
        if (x.slice(-8) == '.pending' || !fs.existsSync(dir + x)) {
            return
        }
        it(x, function () {
...
describe('parse options', function () {
    ...
    describe('cell', function () {
        it('XLSX should generate HTML by default', function () {
            const wb = X.read(fs.readFileSync(paths.cstxlsx), { type: 'binary' })
        ...
        it('XLSX should not generate HTML when requested', function () {
            const wb = X.read(fs.readFileSync(paths.cstxlsx), { type: 'binary', cellHTML: false })
            const ws = wb.Sheets.Sheet1
            each_cell(ws, function (cell) {
        ...
        it('should generate formulae by default', function () {
            FSTPaths.forEach(function (p) {
                const wb = X.read(fs.readFileSync(p), { type: 'binary' })
                let found = false
        ...
        it('should not generate formulae when requested', function () {
            FSTPaths.forEach(function (p) {
                const wb = X.read(fs.readFileSync(p), { type: 'binary', cellFormula: false })
                wb.SheetNames.forEach(function (s) {
                    const ws = wb.Sheets[s]
                    each_cell(ws, function (cell) {
        ...
        it('should not generate formatted text when requested', function () {
            FSTPaths.forEach(function (p) {
                const wb = X.read(fs.readFileSync(p), { type: 'binary', cellText: false })
                wb.SheetNames.forEach(function (s) {
                    const ws = wb.Sheets[s]
                    each_cell(ws, function (cell) {
        ...
        it('should generate number formats when requested', function () {
            NFPaths.forEach(function (p) {
                const wb = X.read(fs.readFileSync(p), { type: 'binary', cellNF: true })
                wb.SheetNames.forEach(function (s) {
                    const ws = wb.Sheets[s]
                    each_cell(ws, function (cell) {
        ...
        it('should not generate cell styles by default', function () {
            [paths.cssxlsx, paths.cssxlsb, paths.cssxls, paths.cssxml].forEach(function (p) {
                const wb = X.read(fs.readFileSync(p), { type: 'binary', WTF: 1 })
                wb.SheetNames.forEach(function (s) {
                    const ws = wb.Sheets[s]
                    each_cell(ws, function (cell) {
        ...
        it('should generate cell styles when requested', function () {
            /* TODO: XLS / XLML */
            [paths.cssxlsx /*, paths.cssxlsb, paths.cssxls, paths.cssxml*/].forEach(function (p) {
                const wb = X.read(fs.readFileSync(p), { type: 'binary', cellStyles: true })
                let found = false
                wb.SheetNames.forEach(function (s) {
                    const ws = wb.Sheets[s]
                    each_cell(ws, function (cell) {
        ...
        it('should generate cell dates when requested', function () {
            DTPaths.forEach(function (p) {
                const wb = X.read(fs.readFileSync(p), { type: 'binary', cellDates: true, WTF: 1 })
                let found = false
                wb.SheetNames.forEach(function (s) {
                    const ws = wb.Sheets[s]
                    each_cell(ws, function (cell) {
    ...
    describe('sheet', function () {
        ...
        it('should generate sheet stubs when requested', function () {
            [paths.mcxlsx, paths.mcxlsb, paths.mcods, paths.mcxls, paths.mcxml].forEach(function (p) {
                const wb = X.read(fs.readFileSync(p), { type: 'binary', sheetStubs: true })
    ...
    describe('book', function () {
        it('bookSheets should not generate sheets', function () {
            [paths.mcxlsx, paths.mcxlsb, paths.mcxls, paths.mcxml].forEach(function (p) {
                const wb = X.read(fs.readFileSync(p), { type: 'binary', bookSheets: true })
        ...
        it('bookProps should not generate sheets', function () {
            [paths.nfxlsx, paths.nfxlsb, paths.nfxls, paths.nfxml].forEach(function (p) {
                const wb = X.read(fs.readFileSync(p), { type: 'binary', bookProps: true })
                assert(typeof wb.Sheets === 'undefined')
            })
        })
        it('bookProps && bookSheets should not generate sheets', function () {
            [paths.lonxlsx, paths.lonxls].forEach(function (p) {
                const wb = X.read(fs.readFileSync(p), { type: 'binary', bookProps: true, bookSheets: true })
                assert(typeof wb.Sheets === 'undefined')
            })
        })
        ...
        it('bookDeps should generate deps (XLSX/XLSB)', function () {
            [paths.fstxlsx, paths.fstxlsb].forEach(function (p) {
                const wb = X.read(fs.readFileSync(p), { type: 'binary', bookDeps: true })
        ...
        it('should not generate VBA by default', function () {
            let wb = X.read(fs.readFileSync(paths.nfxlsx), { type: 'binary' })
        ...
        it('bookVBA should generate vbaraw (XLSX/XLSB)', function () {
            let wb = X.read(fs.readFileSync(paths.nfxlsx), { type: 'binary', bookVBA: true })
            assert(wb.vbaraw)
...
function diffsty(ws, r1, r2) {
    ...
    stykeys.forEach(function (m) {
        ...
        if (styexc.includes(`${r1}|${r2}|${m}`)) {
            c = 1
        } else if (styexc.includes(`${r2}|${r1}|${m}`)) {
            c = 1
        }
...
describe('parse features', function () {
    ...
    describe('comments', function () {
        if (fs.existsSync(paths.swcxlsx)) {
            it('should have comment as part of cell properties', function () {
                ...
                const sheet = 'Sheet1'
                const wb1 = X.read(fs.readFileSync(paths.swcxlsx), { type: 'binary' })
                const wb2 = X.read(fs.readFileSync(paths.swcxlsb), { type: 'binary' })
                const wb3 = X.read(fs.readFileSync(paths.swcxls), { type: 'binary' })
                const wb4 = X.read(fs.readFileSync(paths.swcxml), { type: 'binary' });
        ...
        [
            ...
            it(`${m[0]} stress test`, function () {
                const wb = X.read(fs.readFileSync(m[1]), { type: 'binary' })
                check_comments(wb)
                const ws0 = wb.Sheets.Sheet2
                assert.equal(get_cell(ws0, 'A1').c[0].a, 'Author')
                assert.equal(get_cell(ws0, 'A1').c[0].t, 'Author:\nGod thinks this is good')
    ...
    describe('sheetRows', function () {
        it('should use original range if not set', function () {
            ...
            const wb1 = X.read(fs.readFileSync(paths.fstxlsx), opts)
            const wb2 = X.read(fs.readFileSync(paths.fstxlsb), opts)
            const wb3 = X.read(fs.readFileSync(paths.fstxls), opts)
            const wb4 = X.read(fs.readFileSync(paths.fstxml), opts);
            [wb1, wb2, wb3, wb4].forEach(function (wb) {
                assert.equal(wb.Sheets.Text['!ref'], 'A1:F49')
            })
        })
        it('should adjust range if set', function () {
            const opts = { type: 'binary', sheetRows: 10 }
            const wb1 = X.read(fs.readFileSync(paths.fstxlsx), opts)
            const wb2 = X.read(fs.readFileSync(paths.fstxlsb), opts)
            const wb3 = X.read(fs.readFileSync(paths.fstxls), opts)
            const wb4 = X.read(fs.readFileSync(paths.fstxml), opts);
        ...
        it('should not generate comment cells', function () {
            const opts = { type: 'binary', sheetRows: 10 }
            const wb1 = X.read(fs.readFileSync(paths.cstxlsx), opts)
            const wb2 = X.read(fs.readFileSync(paths.cstxlsb), opts)
            const wb3 = X.read(fs.readFileSync(paths.cstxls), opts)
            const wb4 = X.read(fs.readFileSync(paths.cstxml), opts);
            /* TODO */
            [wb1, wb2 /*, wb3, wb4 */].forEach(function (wb) {
    ...
    describe('defined names', function () {
        [
            /* desc     path        cmnt */
            ['xlsx', paths.dnsxlsx, true], ['xlsb', paths.dnsxlsb, true], ['xls', paths.dnsxls, true],
    ...
    describe('auto filter', function () {
        [
            ['xlsx', paths.afxlsx], ['xlsb', paths.afxlsb], ['xls', paths.afxls], ['xlml', paths.afxml],
            ['ods', paths.afods]
        ].forEach(function (m) {
            it(m[0], function () {
                const wb = X.read(fs.readFileSync(m[1]), { type: 'binary' })
    ...
    describe('should correctly handle styles', function () {
        ...
        it('correct styles', function () {
            const stylesxls = ranges.map(function (r) {
                return rn2(r)[0]
            }).map(function (r) {
                return get_cell(wsxls, r).s
            })
            const stylesxlsx = ranges.map(function (r) {
                return rn2(r)[0]
            }).map(function (r) {
            ...
            for (let i = 0; i != exp.length; ++i) {
                [
                    'fgColor.theme', 'fgColor.raw_rgb', 'bgColor.theme', 'bgColor.raw_rgb',
                    'patternType'
                ].forEach(function (k) {
            ...
            if (typeof before != 'undefined') {
                ...
                it('before', bef)
                    ...
                    it(w, function () {
                        ...
                        const wb2 = X.read(X.write(wb, { bookType: w, type: 'binary' }), { type: 'binary' })
                        Object.keys(baseprops).forEach(function (k) {
...
describe('roundtrip features', function () {
    ...
    describe('should preserve core properties', function () {
        [['xlml', paths.cpxml], ['xlsx', paths.cpxlsx], ['xlsb', paths.cpxlsb]].forEach(function (w) {
            it(w[0], function () {
                const wb1 = X.read(fs.readFileSync(w[1]), { type: 'binary' })
    ...
    describe('should preserve merge cells', function () {
        ['xlsx', 'xlsb', 'xlml', 'ods'].forEach(function (f) {
            it(f, function () {
                const wb1 = X.read(fs.readFileSync(paths.mcxlsx), { type: 'binary' })
    ...
    describe('should preserve dates', function () {
        seq(16).forEach(function (n) {
            ...
            it(`[${a}] -> (${b}) -> [${c}] -> (${d})`, function () {
                const wb1 = X.read(fs.readFileSync(f), { type: 'binary', cellNF: true, cellDates: di, WTF: opts.WTF })
                ...
                if (m[0].t === 'n' && m[1].t === 'n') {
                    ...
                    assert(Math.abs(datenum(parseDate(m[0].v)) - m[1].v) < 0.01)
                }
            })
        })
    })
    ...
    describe('should preserve formulae', function () {
        [['xlml', paths.fstxml], ['xlsx', paths.fstxlsx], ['ods', paths.fstods]].forEach(function (w) {
            it(w[0], function () {
                const wb1 = X.read(fs.readFileSync(w[1]), { type: 'binary', cellFormula: true })
                const wb2 = X.read(X.write(wb1, { bookType: w[0], type: 'binary' }), {
                    cellFormula: true,
                    type: 'binary'
                })
    ...
    describe('should preserve column properties', function () {
        ['xlml', /*'biff2', */'xlsx', 'xlsb', 'slk'].forEach(function (w) {
            it(w, function () {
                ...
                const wb2 = X.read(X.write(wb1, { bookType: w, type: 'binary' }), { type: 'binary', cellStyles: true })
...
describe('invalid files', function () {
    describe('parse', function () {
        [
            ...
            it(`should fail on ${w[0]}`, function () {
                assert.throws(function () {
                    X.read(fs.readFileSync(dir + w[1], { type: 'binary' }), { type: 'binary' })
                })
                assert.throws(function () {
    ...
    describe('write', function () {
        ...
        it('should pass if a sheet is missing', function () {
            const wb = X.read(fs.readFileSync(paths.fstxlsx), { type: 'binary' })
    ...
    const bef = function () {
        data = [
            ...
            [
                'foo',
                'bar',
                parseDate('2014-02-19T14:30:00.000Z'),
                '0.3'
            ],
            [
                'baz',
    ...
    password_files.forEach(function (x) {
        describe(x, function () {
            it('should throw with no password', function () {
                assert.throws(function () {
                    X.read(fs.readFileSync(dir + x), { type: 'binary' })
                })
            })
            ...
            it.skip('should decrypt file', function () {
                const wb = X.read(fs.readFileSync(dir + x), { type: 'binary', password: 'password', WTF: opts.WTF })
            })
        })
    })
})
            ...
            describe(`MFT ${x}`, function () {
                ...
                it('should parse all', function () {
                    for (let j = 1; j != r.length; ++j) {
                        f[j - 1] = X.read(fs.readFileSync(dir + r[0] + r[j]), mfopts)
                    }
                })
