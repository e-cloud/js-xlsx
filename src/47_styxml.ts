...
import { CS2CP } from './42_sstxml'
import { rgb_tint } from './45_styutils'
...
export const parse_sty_xml = function make_pstyx() {
    const numFmtRegex = /<numFmts([^>]*)>.*<\/numFmts>/
    const cellXfRegex = /<cellXfs([^>]*)>.*<\/cellXfs>/
    const fillsRegex = /<fills([^>]*)>.*<\/fills>/
    const fontsRegex = /<fonts([^>]*)>.*<\/fonts>/
    const bordersRegex = /<borders([^>]*)>.*<\/borders>/
