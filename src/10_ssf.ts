...
function isgeneral(s: string, i: number = 0): boolean {
    return s.length >= 7 + i
...
/* Options */
const opts_fmt: Array<Array<any>> = [['date1904', 0], ['output', ''], ['WTF', false]]

function fixopts(o) {
    for (let y = 0; y != opts_fmt.length; ++y) {
...
export function init_table(t: object) {
    t[0] = 'General'
    t[1] = '0'
    t[2] = '0.00'
    t[3] = '#,##0'
...
export const _general_int = general_fmt_int
...
export const _general = general_fmt
    ...
    function write_num_flt(type: string, fmt: string, val: number): string {
        ...
        if (r = fmt.match(/^#,##0\.([#0]*0)$/)) {
            return val < 0
                ...
                : `${commaify('' + (Math.floor(val) + carry(val, r[1].length)))}.${pad0(dec(val, r[1].length), r[1].length)}`
        }
        if (r = fmt.match(/^#,#*,#0/)) {
...
function eval_fmt(fmt: string, v: any, opts: any, flen: number) {
    ...
    if (nstr.length > 0) {
        ...
        for (i = 0; i < out.length; ++i) {
            if (out[i] != null && out[i].t != 't' && out[i].v.includes('.')) {
                decpt = i
                break
            }
        }
        let lasti = out.length
        if (decpt === out.length && !ostr.includes('E')) {
            for (i = out.length - 1; i >= 0; --i) {
                if (out[i] == null || !'n?('.includes(out[i].t)) {
                    continue
                }
        ...
        } else if (decpt !== out.length && !ostr.includes('E')) {
            jj = ostr.indexOf('.') - 1
            for (i = decpt; i >= 0; --i) {
                if (out[i] == null || !'n?('.includes(out[i].t)) {
                    continue
                }
                j = out[i].v.includes('.') && i === decpt ? out[i].v.indexOf('.') - 1 : out[i].v.length - 1
            ...
            for (i = decpt; i < out.length; ++i) {
                if (out[i] == null || !'n?('.includes(out[i].t) && i !== decpt) {
                    continue
                }
                j = out[i].v.includes('.') && i === decpt ? out[i].v.indexOf('.') + 1 : 0
...
function choose_fmt(f: string, v): [number, string] {
    let fmt = split_fmt(f)
    let len = fmt.length
    const lat = fmt[len - 1].indexOf('@')
    if (len < 4 && lat > -1) {
        --len
    }
    if (fmt.length > 4) {
    ...
    if (!fmt[0].includes('[') && !fmt[1].includes('[')) {
        return [len, ff]
    }
...
// todo: isolate the changing data
let table_fmt = {}

export let _table = table_fmt
//export { table_fmt as _table }

export const load = function load_entry(fmt: string, idx: number) {
    table_fmt[idx] = fmt
}
...
export function load_table(tbl: { [n: number]: string }) {
