import { encode_cell, encode_range, safe_decode_range } from './27_csfutils'
import { check_ws_name } from './71_wbcommon'
import { utils } from './90_utils'

utils.consts = utils.consts || {}

function add_consts(R: any[]) {
...
utils.book_append_sheet = function (wb: Workbook, ws: Worksheet, name?: string) {
    if (!name) {
        for (let i = 1; i <= 0xFFFF; ++i) {
            if (!wb.SheetNames.includes(name = `Sheet${i}`)) {
                break
            }
        }
    }
