...
function write_zip_type(wb: Workbook, opts ?: WriteOpts) {
    ...
    if (o.type === 'file') {
        return fs.writeFileSync(o.file, z.generate(oopts))
    }
...
function write_bstr_type(out: string, opts: WriteOpts) {
    switch (opts.type) {
        ...
        case 'binary':
            return out
        case 'file':
            return fs.writeFileSync(opts.file, out, 'binary')
        case 'buffer': {
            if (has_buf) {
...
function write_string_type(out: string, opts: WriteOpts) {
    switch (opts.type) {
        ...
        case 'binary':
            return out
        case 'file':
            return fs.writeFileSync(opts.file, out, 'utf8')
        case 'buffer': {
            if (has_buf) {
                return new Buffer(out, 'utf8')
            } else {
...
function write_binary_type(out, opts: WriteOpts) {
    switch (opts.type) {
        ...
        case 'file':
            return fs.writeFileSync(opts.file, out)
        case 'buffer':
            return out
        default:
            throw new Error(`Unrecognized type ${opts.type}`)
